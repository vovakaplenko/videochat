...
import (
	...
	"errors"
	"fmt"
	"github.com/SpalkLtd/kurento-go"
...
func modifyMessage(msg []byte, originatorUserId string, originatorClientId string, channelId int64, channelName string) ([]byte, error) {
	var v = &PassData{}
	...
	if (channelName == utils.CHANNEL_PREFIX_SIGINALING) {
		if err := modifyWithKurento(v, channelId); err != nil {
			return nil, err
		}
	}
...
func modifyWithKurento(v *PassData, id int64) error {
	// TODO singleton from main.go
	var server, err = kurento.NewConnection("ws://127.0.0.1:8888")
	if err != nil {
		return err
	}
	var pipeline = new(kurento.MediaPipeline) // pipeline per chat(room)
	err = server.Create(pipeline, nil)
	return nil
}
...
func ConfigureCentrifuge(lc fx.Lifecycle, dbs db.DB) *centrifuge.Node {
	...
	node.On().ClientConnected(func(ctx context.Context, client *centrifuge.Client) {
		...
		client.On().Publish(func(e centrifuge.PublishEvent) centrifuge.PublishReply {
			...
			channelId, channelName, err := getChannelId(e.Channel)
			message, err := modifyMessage(e.Data, e.Info.GetUser(), e.Info.GetClient(), channelId, channelName)
			if err != nil {
				Logger.Errorf("Error getting channel id %v", err)
