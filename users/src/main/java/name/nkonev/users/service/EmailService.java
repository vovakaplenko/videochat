...
public class EmailService {
    ...
    public void sendUserConfirmationToken(String email, UserConfirmationToken userConfirmationToken, String login) {
        ...
        String text = registrationTextTemplate
                .replace(REG_LINK_PLACEHOLDER, baseUrl + Constants.Urls.CONFIRM+ "?"+ Constants.Urls.UUID +"=" + userConfirmationToken.getUuid() + "&login=" + URLEncoder
                    .encode(login, StandardCharsets.UTF_8))
    ...
    public void sendPasswordResetToken(
        String email, PasswordResetToken passwordResetToken, String login) {
        SimpleMailMessage msg = new SimpleMailMessage();
        msg.setFrom(from);
        ...
        String text = passwordResetTextTemplate
                .replace(PASSWORD_RESET_LINK_PLACEHOLDER, baseUrl + Constants.Urls.PASSWORD_RESET+ "?"+ Constants.Urls.UUID +"=" + passwordResetToken.getUuid() + "&login=" + URLEncoder
                    .encode(login, StandardCharsets.UTF_8))
                .replace(LOGIN_PLACEHOLDER, login);
        msg.setText(text);
